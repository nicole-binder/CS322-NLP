  ...
  def parse(self, string):
    ...
    while length > number:
      ...
      if fragments[number] in symbol_set:
        ...
        testing_multiple = False
      elif testing_multiple == True:
        max_length = max(symbol_set, key = len)
        i = 0
        for i in range(1, len(max_length)):
          multiple_fragments = ""
          multiple_fragments += fragments[number] + " "
          for i in range(1,i):
            try:
              multiple_fragments += fragments[number + i] + " "
            except:
              break
          multiple_fragments = multiple_fragments[0:len(multiple_fragments)-1]
          if multiple_fragments in symbol_set:
            print("here!!")
            subset.append(multiple_fragments)
